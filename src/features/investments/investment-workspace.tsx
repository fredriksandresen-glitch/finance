...
export function InvestmentWorkspace({ initialAssets }: { initialAssets: AssetWithMetrics[] }) {
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      <Card>
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Priser registreres manuelt i MVP.</p>
        </div>
              ...
              {assets.map((asset) => (
                <tr key={asset.id} className="border-b border-black/5 dark:border-white/5">
                  <td className="py-3">
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400">{asset.symbol}</p>
                  </td>
                  ...
                  <td className={asset.gainLoss >= 0 ? "py-3 text-right text-emerald-600" : "py-3 text-right text-rose-600"}>
      ...
      <Card>
        ...
        <form action={handleSubmit} className="mt-4 grid gap-3">
          ...
          <label className="grid gap-1 text-sm">
            Kategori
            <select name="category" className="rounded-md border border-black/10 bg-transparent px-3 py-2 dark:border-white/10">
